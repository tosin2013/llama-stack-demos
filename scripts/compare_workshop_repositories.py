...
def list_workshop_repositories():
    ...
    try:
        ...
        response = requests.get(f"{api_url}/user/repos", headers=headers, timeout=30)
        ...
        if response.status_code == 200:
            ...
            workshop_repos = [
                repo
                for repo in repos
                if "workshop" in repo["name"].lower() or "test" in repo["name"].lower()
            ]
...
def compare_repositories(repo1_name: str, repo2_name: str):
    ...
    try:
        ...
        repo1_workflow = (
            "Workflow 1"
            if "content/modules/ROOT/nav.adoc" in repo1_files
            else "Workflow 3"
        )
        repo2_workflow = (
            "Workflow 1"
            if "content/modules/ROOT/nav.adoc" in repo2_files
            else "Workflow 3"
        )
        ...
        print(
            f"  - Structure: {'Antora/Showroom' if repo1_workflow == 'Workflow 1' else 'Legacy/Original'}"
        )
        ...
        print(
            f"  - Structure: {'Antora/Showroom' if repo2_workflow == 'Workflow 1' else 'Legacy/Original'}"
        )
        ...
        if repo1_workflow != repo2_workflow:
            ...
            print(
                f"  - {repo1_name}: {repo1_workflow} (showroom template vs original repository)"
            )
            print(
                f"  - {repo2_name}: {repo2_workflow} (showroom template vs original repository)"
            )
...
def main():
    ...
    parser.add_argument(
        "--list-repos", action="store_true", help="List all workshop repositories"
    )
