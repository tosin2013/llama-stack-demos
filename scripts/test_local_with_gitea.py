...
def test_workflow_1_local():
    ...
    try:
        ...
        try:
            ...
            if (
                "Workshop Repository Created:" in creation_result
                or "Repository created successfully" in creation_result
            ):
        ...
        if (
            ...
            and "NON-COMPLIANT" not in compliance_result
        ):
            print(
                f"✅ Repository is COMPLIANT with ADR-0001 (Score: {compliance_score})"
            )
            success = True
        else:
...
def main():
    ...
    try:
        ...
        print("=" * 60)
        print(
            f"Workflow 1 (DDD Hexagonal): {
                '✅ PASSED' if workflow1_repo else '❌ FAILED'}"
        )
        print(
            f"Workflow 3 (Todo Demo): {
                '✅ PASSED' if workflow3_repo else '❌ FAILED'}"
        )
