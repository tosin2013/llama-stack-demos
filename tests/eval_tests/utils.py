...
def add_plot(
    ...
    column_name: str,
    title: str,
    group_by_column: str = "model",
    fig=None,
    ax=None,
    save_filename: bool = True,
):
    """Add a stacked bar chart to the given axes."""
    if group_by_column not in ["model", "expected_tool_call"]:
        raise ValueError(
            "group_by_column must be either 'model' or 'expected_tool_call'."
        )
    ...
    true_counts = (
        df_plot[df_plot[column_name]]
        .groupby(group_by_column)[column_name]
        .count()
        .reindex(categories, fill_value=0)
    )
    false_counts = (
        df_plot[df_plot[column_name] == False]
        .groupby(group_by_column)[column_name]
        .count()
        .reindex(categories, fill_value=0)
    )
    ...
    bars_false = ax.bar(
        x, false_vals, bottom=true_vals, label="False", color="lightgray"
    )
    ...
    ax.set_xticks(x)
    ax.set_xticklabels(
        categories, rotation=rotation, ha="right", fontsize=fontsize_xticks
    )
