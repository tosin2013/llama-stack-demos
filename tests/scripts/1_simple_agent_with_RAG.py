...
parser.add_argument(
    "-a",
    "--auto",
    help="Automatically runs examples, and does not start a chat session",
    action="store_true",
)
...
vector_db_ids = [
    vector_db.provider_resource_id for vector_db in client.vector_dbs.list()
]
if "my_documents" not in vector_db_ids:
    ...
    urls = [
        "https://raw.githubusercontent.com/meta-llama/llama-stack/refs/heads/main/README.md"
    ]
    documents = [
        Document(
            document_id=f"num-{i}",
    ...
    while True:
        ...
        if "/bye" in user_input:
            if args.session_info_on_exit:
                agent_session = client.agents.session.retrieve(
