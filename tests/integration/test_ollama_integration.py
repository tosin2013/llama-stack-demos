...
class TestOllamaIntegration:
    ...
    def test_ollama_simple_generation(self, ollama_available):
        ...
        payload = {"model": model, "prompt": "What is a workshop?", "stream": False}
    ...
    def test_repository_analysis_prompts(self, ollama_available, test_repositories):
        ...
        payload = {"model": model, "prompt": analysis_prompt, "stream": False}
        ...
        assert any(
            ...
            for keyword in ["workshop", "learn", "objective", "skill"]
    ...
    def test_ollama_workshop_content_generation(self, ollama_available):
        ...
        payload = {"model": model, "prompt": content_prompt, "stream": False}
    ...
    def test_ollama_error_handling(self, ollama_available):
        ...
        response = requests.post(
            f"{endpoint}/api/generate", json=invalid_payload, timeout=10
        )
    ...
    def test_ollama_streaming_response(self, ollama_available):
        ...
        response = requests.post(
            f"{endpoint}/api/generate", json=payload, stream=True, timeout=30
        )
    ...
    def test_ollama_performance_benchmark(self, ollama_available):
        ...
        ), f"Average response time too slow: {
            avg_response_time:.2f}s"
        assert (
            max_response_time < 60
        ), f"Max response time too slow: {
            max_response_time:.2f}s"

        print(
            f"✅ Performance benchmark: avg={
                avg_response_time:.2f}s, max={
                max_response_time:.2f}s"
        )
