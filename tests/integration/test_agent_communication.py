...
import pytest
import time
from tests.utils.agent_client import WorkshopSystemClient, AgentClient
...
class TestAgentCommunication:
    ...
    def test_agent_card_retrieval(self, agent_endpoints):
        ...
        print(f"✅ Successfully retrieved agent cards from all {len(agent_cards)} agents")
    ...
    def test_sequential_agent_workflow(self, agent_endpoints, test_repositories):
        ...
        template_result = template_converter.send_message(
            f"Convert to workshop template based on analysis: {source_result.data}"
        )
        ...
        content_result = content_creator.send_message(
            f"Create workshop content based on template: {template_result.data}"
        )
    ...
    def test_parallel_agent_requests(self, agent_endpoints):
        ...
        print(f"✅ All {len(results)} agents handled parallel requests successfully")
    ...
    def test_agent_session_management(self, agent_endpoints):
        ...
        for agent_name, agent in client.agents.items():
            response = agent.send_message(
                f"Hello from {agent_name} session test",
    ...
    def test_agent_load_handling(self, agent_endpoints):
        ...
        for agent_name, agent in client.agents.items():
            ...
            print(f"✅ Agent {agent_name} handled load test: {success_rate:.2%} success rate")
    ...
    def test_agent_response_consistency(self, agent_endpoints):
        ...
        for agent_name, agent in client.agents.items():
            ...
            success_statuses = [r.success if hasattr(r, 'success') else (r.status_code == 200) for r in responses]
