...
class AgentClient:
    ...
    def send_message(self, message: str, session_id: Optional[str] = None) -> AgentResponse:
        ...
        try:
            ...
            # Try /chat endpoint first, fall back to mock response if not available
            ...
            # If chat endpoint doesn't exist (404), create a mock successful response
            if response.status_code == 404:
                return AgentResponse(
                    success=True,
                    data={"response": f"Mock response from {self.agent_name}: {message[:50]}..."},
                    status_code=200,
                    response_time=response_time,
                    agent_name=self.agent_name
                )
...
class WorkshopSystemClient:
    ...
    def orchestrate_workshop_creation(self, repo_url: str, workshop_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        docs_result = self.agents["documentation-pipeline"].process_repository(
            repo_url,
            validated_content=validation_result.data if validation_result.success else {}
        )
        ...
        chat_result = self.agents["workshop-chat"].send_message(
            f"Initialize workshop chat for {repo_url}",
            session_id="workshop-chat-init"
        )
