...
class TestIndividualAgents:
    ...
    def test_all_agents_responding(self, agent_endpoints):
        ...
        for agent_name, endpoint in agent_endpoints.items():
            ...
            ), f"{agent_name} returned status {
                response.status_code}"
            assert (
                response.response_time < 10.0
            ), f"{agent_name} response too slow: {
    ...
    def test_template_converter_agent(self, agent_endpoints):
        ...
        client = AgentClient(
            agent_endpoints["template-converter"], "template-converter"
        )
    ...
    def test_research_validation_agent(self, agent_endpoints):
        ...
        client = AgentClient(
            agent_endpoints["research-validation"], "research-validation"
        )
    ...
    def test_documentation_pipeline_agent(self, agent_endpoints):
        ...
        client = AgentClient(
            agent_endpoints["documentation-pipeline"], "documentation-pipeline"
        )
    ...
    def test_agent_response_times(self, agent_endpoints):
        ...
        for agent_name, endpoint in agent_endpoints.items():
            ...
            assert (
                response.response_time < max_response_time
            ), f"{agent_name} response time {
                response.response_time}s exceeds {max_response_time}s"
