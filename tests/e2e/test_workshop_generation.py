...
class TestWorkshopGeneration:
    ...
    def test_full_workshop_creation_workflow(self, agent_endpoints, test_repositories, sample_workshop_request):
        ...
        print(f"\n🚀 Starting full workshop creation workflow for {test_repo['name']}")
        
        # Execute the full workflow
        workflow_results = client.orchestrate_workshop_creation(
            test_repo["url"],
            sample_workshop_request
        )
        ...
        for step_name, result in workflow_results.items():
            assert hasattr(result, 'status_code'), f"Step {step_name} missing status code"
            assert result.status_code in [200, 404, 501], f"Step {step_name} failed with status {result.status_code}"
        
        print("✅ Full workshop creation workflow completed")
        return workflow_results
    
    def test_workshop_creation_with_different_repositories(self, agent_endpoints, test_repositories):
        """Test workshop creation with different types of repositories."""
        client = WorkshopSystemClient(agent_endpoints)
        
        for repo in test_repositories:
            print(f"\n🔍 Testing workshop creation for {repo['name']} ({repo['type']})")
    ...
    def test_workshop_creation_error_handling(self, agent_endpoints):
        ...
        workflow_results = client.orchestrate_workshop_creation(
            invalid_repo_url,
            {"workshop_title": "Invalid Repo Test"}
        )
        ...
        assert completed_steps >= 3, "At least half the steps should handle errors gracefully"
    ...
    def test_workshop_creation_performance(self, agent_endpoints, test_repositories):
        ...
        for step_name, result in workflow_results.items():
            if hasattr(result, 'response_time'):
                assert result.response_time < 30, f"Step {step_name} took too long: {result.response_time:.2f}s"
    ...
    def test_workshop_customization_options(self, agent_endpoints, test_repositories):
        ...
        for i, config in enumerate(configurations):
            print(f"\n🎯 Testing configuration {i+1}: {config['workshop_title']}")
            ...
            # Validate workflow completed
            assert len(workflow_results) == 6, f"Configuration {i+1} workflow incomplete"
            
            # Check that configuration was processed
            for step_name, result in workflow_results.items():
                assert hasattr(result, 'status_code'), f"Configuration {i+1} step {step_name} missing status"
    ...
    def test_workshop_state_persistence(self, agent_endpoints, test_repositories):
        ...
        workflow_results = client.orchestrate_workshop_creation(
            test_repo["url"],
            {
                "workshop_title": "Persistence Test Workshop",
        ...
        followup_response = chat_agent.send_message(
            "What workshop did we just create?",
            session_id=session_id
        )
        
        # Agent should handle the session request (even if with mock response)
        assert followup_response.status_code in [200, 404, 501], \
