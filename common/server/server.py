        ...
        try:
            ...
            try:
                ...
                task_response = asyncio.run(
                    self.task_manager.on_send_task(task_request)
                )
                ...
                response = {
                    ...
                    "result": {
                        ...
                        "artifacts": (
                            ...
                            if task_response.result.artifacts
                            else []
                        ),
                    },
                }
                        ...
                        "status": {
                            ...
                            "message": {
                                ...
                                "parts": [
                                    {
                                        "type": "text",
                                        "text": f"Task processing error: {str(e)}",
                                    }
                                ],
                            },
                        },
        ...
        try:
            ...
            def handler_factory(*args, **kwargs):
                return A2ARequestHandler(
                    self.agent_card, self.task_manager, *args, **kwargs
                )

            self.server = HTTPServer((self.host, self.port), handler_factory)

            logger.info(
                f"Starting A2A server for agent: {
                    self.agent_card.name}"
            )
            logger.info(f"Server running on http://{self.host}:{self.port}")
            logger.info(
                f"Agent card available at http://{self.host}:{self.port}/agent-card"
            )
