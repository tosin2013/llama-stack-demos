...
export async function generateResearchQuestions(args) {
    const { analysisType = 'comprehensive', researchContext, problems, knowledgeGraph, relevantKnowledge, researchQuestions, currentProgress, projectPath = process.cwd(), adrDirectory = 'docs/adrs', } = args;
    try {
        const { correlateProblemKnowledge, findRelevantAdrPatterns, generateContextAwareQuestions, createResearchTaskTracking, } = await import('../utils/research-questions.js');
        switch (analysisType) {
            case 'correlation': {
                if (!problems || !knowledgeGraph) {
                    throw new McpAdrError('Problems and knowledge graph are required for correlation analysis', 'INVALID_INPUT');
                }
...
`,
            ...
            case 'questions': {
                ...
                if (!relevantKnowledge) {
                    throw new McpAdrError('Relevant knowledge is required for question generation. Please run relevance analysis first.', 'INVALID_INPUT');
                }
                const result = await generateContextAwareQuestions({
                    ...effectiveResearchContext,
                    constraints: effectiveResearchContext.constraints || [],
                    timeline: effectiveResearchContext.timeline || 'Not specified',
                }, relevantKnowledge, projectPath);
                ...
                const executionResult = await executeResearchPrompt(result.questionPrompt, result.instructions, {
...
`,
            ...
            case 'tracking': {
                if (!researchQuestions) {
                    throw new McpAdrError('Research questions are required for task tracking', 'INVALID_INPUT');
                }
                const result = await createResearchTaskTracking(researchQuestions.map(q => ({
                    ...q,
                    complexity: 'medium',
...
`,
        ...
        throw new McpAdrError(`Failed to generate research questions: ${error instanceof Error ? error.message : String(error)}`, 'RESEARCH_QUESTION_ERROR');
    }
}
//# sourceMappingURL=research-question-tool.js.map