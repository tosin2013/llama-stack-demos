...
export async function analyzeContentSecurity(args) {
    ...
    try {
        ...
        const executionResult = await executePromptWithFallback(result.analysisPrompt, result.instructions, {
            temperature: 0.1,
            maxTokens: 4000,
...
`,
        ...
        throw new McpAdrError(`Failed to analyze content security: ${error instanceof Error ? error.message : String(error)}`, 'ANALYSIS_ERROR');
    }
}
/**
...
export async function generateContentMasking(args) {
    ...
    try {
        ...
        const { executePromptWithFallback, formatMCPResponse } = await import('../utils/prompt-execution.js');
        const executionResult = await executePromptWithFallback(result.maskingPrompt, result.instructions, {
            temperature: 0.1,
            maxTokens: 4000,
...
`;
        ...
        throw new McpAdrError(`Failed to configure custom patterns: ${error instanceof Error ? error.message : String(error)}`, 'CONFIGURATION_ERROR');
    }
}
/**
...
`,
        ...
        throw new McpAdrError(`Failed to apply basic masking: ${error instanceof Error ? error.message : String(error)}`, 'MASKING_ERROR');
    }
}
/**
...
`,
        ...
        throw new McpAdrError(`Failed to validate masking: ${error instanceof Error ? error.message : String(error)}`, 'VALIDATION_ERROR');
    }
}
//# sourceMappingURL=content-masking-tool.js.map