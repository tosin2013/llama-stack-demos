...
export async function analyzeEnvironment(args) {
    const { projectPath = process.cwd(), adrDirectory = 'docs/adrs', analysisType = 'comprehensive', currentEnvironment, requirements, industryStandards, } = args;
    try {
        const { analyzeEnvironmentSpecs, detectContainerization, determineEnvironmentRequirements, assessEnvironmentCompliance, } = await import('../utils/environment-analysis.js');
        switch (analysisType) {
            case 'specs': {
...
`,
            ...
            case 'compliance': {
                if (!currentEnvironment || !requirements) {
                    throw new McpAdrError('Current environment and requirements are required for compliance assessment', 'INVALID_INPUT');
                }
                const result = await assessEnvironmentCompliance(currentEnvironment, requirements, industryStandards);
                return {
                    content: [
                        {
                            type: 'text',
...
`,
        ...
        throw new McpAdrError(`Failed to analyze environment: ${error instanceof Error ? error.message : String(error)}`, 'ENVIRONMENT_ANALYSIS_ERROR');
    }
}
//# sourceMappingURL=environment-analysis-tool.js.map