...
export async function generateRules(args) {
    const { source = 'both', adrDirectory = 'docs/adrs', projectPath = process.cwd(), existingRules, outputFormat = 'json', } = args;
    try {
        const { extractRulesFromAdrs, generateRulesFromPatterns } = await import('../utils/rule-generation.js');
        switch (source) {
            case 'adrs': {
                ...
                const executionResult = await executePromptWithFallback(result.extractionPrompt, result.instructions, {
                    temperature: 0.1,
                    maxTokens: 6000,
...
`,
            ...
            case 'patterns': {
                ...
                const { executePromptWithFallback, formatMCPResponse } = await import('../utils/prompt-execution.js');
                const executionResult = await executePromptWithFallback(result.generationPrompt, result.instructions, {
                    temperature: 0.1,
                    maxTokens: 6000,
...
`,
            ...
            case 'both': {
                const adrResult = await extractRulesFromAdrs(adrDirectory, existingRules);
                const existingRuleNames = existingRules?.map(r => r.name);
...
`;
                ...
                const { executePromptWithFallback, formatMCPResponse } = await import('../utils/prompt-execution.js');
                const executionResult = await executePromptWithFallback(comprehensivePrompt, `${adrResult.instructions}\n\n${patternResult.instructions}`, {
                    temperature: 0.1,
                    maxTokens: 8000,
...
`,
        ...
        throw new McpAdrError(`Failed to generate rules: ${error instanceof Error ? error.message : String(error)}`, 'RULE_GENERATION_ERROR');
    }
}
/**
...
export async function validateRules(args) {
    const { filePath, fileContent, fileName, rules, validationType = 'file', reportFormat = 'detailed', } = args;
    try {
        ...
        const { executePromptWithFallback, formatMCPResponse } = await import('../utils/prompt-execution.js');
        const executionResult = await executePromptWithFallback(result.validationPrompt, result.instructions, {
            temperature: 0.1,
            maxTokens: 5000,
...
`,
        ...
        throw new McpAdrError(`Failed to validate rules: ${error instanceof Error ? error.message : String(error)}`, 'RULE_VALIDATION_ERROR');
    }
}
/**
...
export async function createRuleSet(args) {
    const { name, description = 'Generated architectural rule set', adrRules = [], patternRules = [], rules = [], outputFormat = 'json', author = 'MCP ADR Analysis Server', } = args;
    try {
        const { createRuleSet, serializeRuleSetToJson, serializeRuleSetToYaml } = await import('../utils/rule-format.js');
        // Combine all rules
...
`,
        ...
        throw new McpAdrError(`Failed to create rule set: ${error instanceof Error ? error.message : String(error)}`, 'RULE_SET_CREATION_ERROR');
    }
}
//# sourceMappingURL=rule-generation-tool.js.map