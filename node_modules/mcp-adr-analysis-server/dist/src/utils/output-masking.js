 ...
 */
export async function maskMcpResponse(response, config = DEFAULT_MASKING_CONFIG) {
    if (!config.enabled) {
        return response;
    }
    try {
        ...
        throw new McpAdrError(`Failed to mask MCP response: ${error instanceof Error ? error.message : String(error)}`, 'MASKING_ERROR');
    }
}
/**
 * Apply content masking to a text string
 */
async function maskContent(content, config) {
    try {
 ...
 */
export async function generateAiMasking(content, contentType = 'general') {
    try {
...
`,
        ...
        throw new McpAdrError(`Failed to generate AI masking: ${error instanceof Error ? error.message : String(error)}`, 'MASKING_ERROR');
    }
}
/**
 * Create masking configuration from environment or defaults
 */
export function createMaskingConfig(overrides) {
    const envConfig = {
        enabled: process.env['MCP_MASKING_ENABLED'] !== 'false',
        strategy: process.env['MCP_MASKING_STRATEGY'] || 'partial',
    };
    return {
 ...
 */
export function validateMaskingConfig(config) {
    const errors = [];
 ...
 */
export function withContentMasking(toolFunction, config) {
    return (async (...args) => {
 ...
 */
export async function applyProgressiveMasking(content, sensitivityLevel = 'medium') {
    const strategies = {
        low: 'placeholder',
        medium: 'partial',
        high: 'full',
 ...
 */
export function detectContentSensitivity(content) {
...
//# sourceMappingURL=output-masking.js.map