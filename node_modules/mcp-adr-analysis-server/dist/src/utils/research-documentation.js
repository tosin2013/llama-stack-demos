 ...
 */
export async function createResearchDocument(researchData, outputDirectory = 'docs/research') {
    try {
        ...
        throw new McpAdrError(`Failed to create research document: ${error instanceof Error ? error.message : String(error)}`, 'RESEARCH_DOCUMENT_ERROR');
    }
}
/**
 * Generate research document content in markdown format
 */
function generateResearchDocumentContent(research) {
 ...
 */
export async function createResearchIndex(researchDocuments, outputDirectory = 'docs/research') {
    try {
        const { promises: fs } = await import('fs');
        const { join } = await import('path');
        // Ensure output directory exists
        ...
        throw new McpAdrError(`Failed to create research index: ${error instanceof Error ? error.message : String(error)}`, 'RESEARCH_INDEX_ERROR');
    }
}
/**
 * Generate research index content
 */
function generateResearchIndexContent(researchDocuments) {
    const now = new Date().toISOString().split('T')[0];
    // Group by status
...
//# sourceMappingURL=research-documentation.js.map