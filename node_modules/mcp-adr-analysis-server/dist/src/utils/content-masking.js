 ...
 */
export async function analyzeSensitiveContent(content, contentType = 'general', userDefinedPatterns) {
    try {
        const { generateSensitiveContentDetectionPrompt } = await import('../prompts/security-prompts.js');
        const analysisPrompt = generateSensitiveContentDetectionPrompt(content, contentType, userDefinedPatterns);
        const instructions = `
...
`;
        ...
        throw new McpAdrError(`Failed to generate sensitive content analysis: ${error instanceof Error ? error.message : String(error)}`, 'ANALYSIS_ERROR');
    }
}
/**
 * Generate content masking prompt for AI processing
 */
export async function generateMaskingInstructions(content, detectedSensitiveItems, maskingStrategy = 'full') {
    try {
        const { generateContentMaskingPrompt } = await import('../prompts/security-prompts.js');
        const maskingPrompt = generateContentMaskingPrompt(content, detectedSensitiveItems, maskingStrategy);
        const instructions = `
...
`;
        ...
        throw new McpAdrError(`Failed to generate masking instructions: ${error instanceof Error ? error.message : String(error)}`, 'MASKING_ERROR');
    }
}
/**
 * Generate custom pattern configuration prompt
 */
export async function generateCustomPatternConfiguration(projectContext, existingPatterns) {
    try {
        const { generateCustomPatternConfigurationPrompt } = await import('../prompts/security-prompts.js');
        const configurationPrompt = generateCustomPatternConfigurationPrompt(projectContext, existingPatterns);
        const instructions = `
...
`;
        ...
        throw new McpAdrError(`Failed to generate pattern configuration: ${error instanceof Error ? error.message : String(error)}`, 'CONFIGURATION_ERROR');
    }
}
/**
 * Apply basic masking patterns (fallback when AI is not available)
 */
export function applyBasicMasking(content, maskingStrategy = 'full') {
 ...
 */
export function validateMasking(originalContent, maskedContent) {
    const issues = [];
...
//# sourceMappingURL=content-masking.js.map