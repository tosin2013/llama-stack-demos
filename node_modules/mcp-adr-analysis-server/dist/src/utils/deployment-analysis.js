 ...
 */
export async function identifyDeploymentTasks(adrDirectory = 'docs/adrs', todoPath) {
    try {
...
`;
        ...
        throw new McpAdrError(`Failed to identify deployment tasks: ${error instanceof Error ? error.message : String(error)}`, 'DEPLOYMENT_TASK_IDENTIFICATION_ERROR');
    }
}
/**
 * Analyze CI/CD logs and pipeline status
 */
export async function analyzeCiCdStatus(cicdLogs, pipelineConfig, deploymentTasks) {
    try {
...
`;
        ...
        throw new McpAdrError(`Failed to analyze CI/CD status: ${error instanceof Error ? error.message : String(error)}`, 'CICD_ANALYSIS_ERROR');
    }
}
/**
 * Calculate deployment progress
 */
export async function calculateDeploymentProgress(deploymentTasks, cicdStatus, environmentStatus) {
    try {
        ...
        const progressPrompt = generateDeploymentProgressCalculationPrompt(taskData, cicdStatus, environmentStatus);
        const instructions = `
...
`;
        ...
        throw new McpAdrError(`Failed to calculate deployment progress: ${error instanceof Error ? error.message : String(error)}`, 'PROGRESS_CALCULATION_ERROR');
    }
}
/**
 * Verify deployment completion with outcome rules
 */
export async function verifyDeploymentCompletion(deploymentTasks, outcomeRules, actualOutcomes) {
    try {
        ...
        const verificationPrompt = generateCompletionVerificationPrompt(taskData, outcomeRules, actualOutcomes);
        const instructions = `
...
`;
        ...
        throw new McpAdrError(`Failed to verify deployment completion: ${error instanceof Error ? error.message : String(error)}`, 'COMPLETION_VERIFICATION_ERROR');
    }
}
//# sourceMappingURL=deployment-analysis.js.map