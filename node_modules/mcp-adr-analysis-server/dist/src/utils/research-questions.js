...
export async function generateContextAwareQuestions(researchContext, relevantKnowledge, projectPath = process.cwd()) {
    try {
        ...
        const projectContext = {
            technologies: projectStructure.detectedTechnologies || [],
...
function inferArchitectureType(projectStructure) {
    if (projectStructure.directories?.some((dir) => dir.includes('service'))) {
        return 'microservices';
    }
    if (projectStructure.directories?.some((dir) => dir.includes('layer'))) {
        return 'layered';
    }
...
function inferDomainType(projectStructure) {
    ...
    if (technologies.some((tech) => tech.includes('react') || tech.includes('vue') || tech.includes('angular'))) {
        return 'web-application';
    }
    if (technologies.some((tech) => tech.includes('api') || tech.includes('rest'))) {
        return 'api-service';
    }
    if (technologies.some((tech) => tech.includes('data') || tech.includes('analytics'))) {
