 ...
 */
export async function monitorResearchDirectory(researchPath = 'docs/research') {
    try {
        const { findFiles } = await import('./file-system.js');
        // Generate research file discovery prompt for AI delegation
        const researchFileDiscoveryPrompt = await findFiles(process.cwd(), [
            `${researchPath}/**/*.md`,
            `${researchPath}/**/*.txt`,
            `${researchPath}/**/*.pdf`,
            `${researchPath}/**/*.doc`,
...
`;
        ...
        throw new McpAdrError(`Failed to monitor research directory: ${error instanceof Error ? error.message : String(error)}`, 'MONITORING_ERROR');
    }
}
/**
 * Extract research topics from files
 */
export async function extractResearchTopics(researchPath = 'docs/research', existingTopics) {
    try {
        const { findFiles } = await import('./file-system.js');
        // Generate research file discovery prompt for AI delegation
        const researchFileDiscoveryPrompt = await findFiles(process.cwd(), [`${researchPath}/**/*.md`, `${researchPath}/**/*.txt`], { includeContent: true });
...
`;
        ...
        throw new McpAdrError(`Failed to extract research topics: ${error instanceof Error ? error.message : String(error)}`, 'EXTRACTION_ERROR');
    }
}
/**
 * Evaluate research impact on existing ADRs
 */
export async function evaluateResearchImpact(researchTopics, adrDirectory = 'docs/adrs') {
    try {
        const { findFiles } = await import('./file-system.js');
...
`;
        ...
        throw new McpAdrError(`Failed to evaluate research impact: ${error instanceof Error ? error.message : String(error)}`, 'EVALUATION_ERROR');
    }
}
/**
 * Generate ADR update suggestions
 */
export async function generateAdrUpdateSuggestions(adrId, researchFindings, updateType, adrDirectory = 'docs/adrs') {
    try {
        const { findFiles } = await import('./file-system.js');
        // Generate ADR file discovery prompt for AI delegation
        const adrFileDiscoveryPrompt = await findFiles(process.cwd(), [`${adrDirectory}/**/*.md`], {
...
`;
        ...
        throw new McpAdrError(`Failed to generate ADR update suggestions: ${error instanceof Error ? error.message : String(error)}`, 'UPDATE_GENERATION_ERROR');
    }
}
/**
 * Create research file template
 */
export function createResearchTemplate(title, category = 'general') {
 ...
 */
export function promptForActionConfirmation(action, details, impact) {
...
//# sourceMappingURL=research-integration.js.map