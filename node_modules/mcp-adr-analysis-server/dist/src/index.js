...
export class McpAdrAnalysisServer {
    ...
    setupHandlers() {
        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
            ...
            try {
                ...
                throw new McpAdrError(`Tool execution failed: ${error instanceof Error ? error.message : String(error)}`, 'TOOL_EXECUTION_ERROR');
            }
        });
        // Resources handler
        ...
        this.server.setRequestHandler(ReadResourceRequestSchema, async (request) => {
            ...
            try {
                ...
                throw new McpAdrError(`Resource reading failed: ${error instanceof Error ? error.message : String(error)}`, 'RESOURCE_ERROR');
            }
        });
        // Prompts handler
        ...
        this.server.setRequestHandler(GetPromptRequestSchema, async (request) => {
            ...
            try {
                ...
                throw new McpAdrError(`Prompt execution failed: ${error instanceof Error ? error.message : String(error)}`, 'PROMPT_EXECUTION_ERROR');
            }
        });
    }
    /**
     ...
     */
    async checkAIExecutionStatus(_args) {
        try {
...
`
    ...
    async getWorkflowGuidance(args) {
        const { goal, projectContext, availableAssets = [], timeframe = 'thorough_review', primaryConcerns = [] } = args;
        try {
...
`;
            ...
            const executionResult = await executePromptWithFallback(workflowPrompt, 'Analyze the user context and provide intelligent workflow guidance with specific tool recommendations.', {
                temperature: 0.1,
                maxTokens: 6000,
...
`,
            ...
            throw new McpAdrError(`Failed to generate workflow guidance: ${error instanceof Error ? error.message : String(error)}`, 'WORKFLOW_ERROR');
        }
    }
    async getDevelopmentGuidance(args) {
        const { developmentPhase, adrsToImplement = [], technologyStack = [], currentProgress = '', teamContext = { size: 'small_team', experienceLevel: 'mixed' }, timeline = '', focusAreas = [] } = args;
        try {
...
`;
            ...
            const { executePromptWithFallback, formatMCPResponse } = await import('./utils/prompt-execution.js');
            const executionResult = await executePromptWithFallback(developmentPrompt, 'Analyze the development context and provide comprehensive implementation guidance that translates architectural decisions into specific coding tasks and development roadmap.', {
                temperature: 0.1,
                maxTokens: 8000,
...
`,
            ...
            throw new McpAdrError(`Failed to generate development guidance: ${error instanceof Error ? error.message : String(error)}`, 'DEVELOPMENT_ERROR');
        }
    }
    async analyzeProjectEcosystem(args) {
        // Use configured project path if not provided in args
        const projectPath = args.projectPath || this.config.projectPath;
        const { includePatterns, enhancedMode = true, knowledgeEnhancement = true, learningEnabled = true, technologyFocus = [], analysisDepth = 'comprehensive', includeEnvironment = true, recursiveDepth = 'comprehensive', analysisScope = [] } = args;
        ...
        try {
            ...
            let generateArchitecturalKnowledge = null;
            let executeWithReflexion = null;
            let retrieveRelevantMemories = null;
            let createToolReflexionConfig = null;
            if (enhancedMode) {
...
`;
            ...
            if (enhancedMode && learningEnabled && retrieveRelevantMemories) {
                try {
                    const memoryResult = await retrieveRelevantMemories('ecosystem-analysis', { projectPath, analysisDepth, technologyFocus }, { maxResults: 5, relevanceThreshold: 0.6 });
                    reflexionContext = `
...
`;
            ...
            const executionResult = await executeEcosystemAnalysisPrompt(enhancedAnalysisPrompt, baseProjectAnalysisPrompt.instructions, {
                temperature: 0.1,
                maxTokens: 6000,
            });
            if (executionResult.isAIGenerated) {
...
- Validate recommendations align with domain best practices`
            ...
            throw new McpAdrError(`Failed to analyze project ecosystem: ${error instanceof Error ? error.message : String(error)}`, 'ANALYSIS_ERROR');
        }
    }
    async getArchitecturalContext(args) {
...
`;
            // Execute the analysis with AI if enabled, otherwise return prompt
            const { executeEcosystemAnalysisPrompt, formatMCPResponse } = await import('./utils/prompt-execution.js');
            const executionResult = await executeEcosystemAnalysisPrompt(architecturalPrompt, projectAnalysisPrompt.instructions, {
                temperature: 0.1,
                maxTokens: 5000,
...
`,
            ...
            throw new McpAdrError(`Failed to get architectural context: ${error instanceof Error ? error.message : String(error)}`, 'ANALYSIS_ERROR');
        }
    }
    async generateAdrsFromPrd(args) {
        const { prdPath, enhancedMode = true, promptOptimization = true, knowledgeEnhancement = true, prdType = 'general' } = args;
        ...
        try {
            ...
            // Import advanced prompting utilities if enhanced mode is enabled
            let generateArchitecturalKnowledge = null;
            let optimizePromptWithAPE = null;
            let createToolAPEConfig = null;
            if (enhancedMode) {
                if (knowledgeEnhancement) {
                    const knowledgeModule = await import('./utils/knowledge-generation.js');
                    generateArchitecturalKnowledge = knowledgeModule.generateArchitecturalKnowledge;
...
`;
            ...
            const executionResult = await executeADRGenerationPrompt(adrPrompt, 'Generate comprehensive ADRs from PRD analysis with enhanced domain knowledge and optimization', {
                temperature: 0.1,
                maxTokens: 8000,
...
The enhanced process maintains full traceability from PRD requirements to generated ADRs while providing superior quality through advanced prompting techniques and ensuring security and user control over file operations.`
            ...
            throw new McpAdrError(`Failed to generate ADR prompts from PRD: ${error instanceof Error ? error.message : String(error)}`, 'PROMPT_GENERATION_ERROR');
        }
    }
    async generateAdrTodo(args) {
...
`;
            ...
            const { executePromptWithFallback, formatMCPResponse } = await import('./utils/prompt-execution.js');
            const executionResult = await executePromptWithFallback(todoPrompt, 'Generate comprehensive todo.md file from ADR analysis with task extraction and progress tracking', {
                temperature: 0.1,
                maxTokens: 6000,
...
The AI agent will safely remove the specified cache entry.`
            ...
            throw new McpAdrError(`Cache management failed: ${error instanceof Error ? error.message : String(error)}`, 'CACHE_ERROR');
        }
    }
    async configureOutputMasking(args) {
...
`
                ...
                case 'set': {
                    ...
                    if (!validation.isValid) {
                        throw new McpAdrError(`Invalid masking configuration: ${validation.errors.join(', ')}`, 'INVALID_CONFIG');
                    }
...
`
            ...
            throw new McpAdrError(`Output masking configuration failed: ${error instanceof Error ? error.message : String(error)}`, 'CONFIG_ERROR');
        }
    }
    /**
     * Helper method to get domains based on PRD type
     */
    getPrdTypeDomains(prdType) {
        const domainMap = {
     ...
     */
    getEcosystemAnalysisDomains(technologyFocus) {
     ...
     */
    createBaseAdrPrompt(prdPath, outputDirectory, knowledgeContext) {
        return `
# Enhanced ADR Generation from PRD Request
...
`;
     ...
     */
    async generateResearchQuestions(args) {
     ...
     */
    async analyzeDeploymentProgress(args) {
     ...
     */
    async applyOutputMasking(response) {
        try {
    ...
    async readResource(uri) {
        try {
            ...
            throw new McpAdrError(`Failed to read resource: ${error instanceof Error ? error.message : String(error)}`, 'RESOURCE_ERROR');
        }
    }
    /**
     * Start the server
