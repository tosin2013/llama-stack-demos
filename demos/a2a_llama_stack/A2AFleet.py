...
from llama_stack_client.types import ResponseFormat, SamplingParams
from llama_stack_client.types.shared_params.agent_config import ToolConfig, Toolgroup
...
class AgentSpecification(BaseModel):
    ...
    def validate_specification(self):
        if self.a2a_agent_card is None and self.url is None:
            raise ValueError(
                "Either the agent URL or the agent A2A card must be provided."
            )
        ...
        if self.managed:
            if self.a2a_agent_card is None:
                raise ValueError(
                    "The agent A2A card must be provided for a managed agent."
                )
            if self.lls_agent_config is None:
                raise ValueError(
                    "The Llama Stack agent configuration must be provided for a managed agent."
                )
...
class A2AFleetAgent:
    def __init__(self, agent_specification: AgentSpecification):
        ...
        # after the client tool initialization, we have the agent card even if
        # it was not given at init
        self.spec.a2a_agent_card = self.client_tool_method.agent_card

        # if this is a managed agent, the LLS agent object and the A2A server
        # wrapper will be initialized later
        self.lls_agent = None
...
class A2AFleet:
    ...
    def query_agent(self, agent_id, **kwargs):
        ...
        if not agent.spec.managed:
            raise ValueError(
                f"Agent {agent_id} is an external A2A agent and cannot be queried via this interface."
            )
...
class FullMeshA2AFleet(A2AFleet):
    ...
    def run_fleet(self):
        # before actually running the agents, modify their tool lists to
        # include the peer client methods
        for agent in self.agents.values():
            if not agent.spec.managed:
                continue
            client_tools = [
                a.client_tool_method for a in self.agents.values() if a != agent
            ]
            if not client_tools:
                continue
