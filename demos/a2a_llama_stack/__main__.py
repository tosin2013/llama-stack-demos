...
def build_server(agent_name: str, host: str, port: int = None):
    ...
    agent = Agent(
        ...
        model=os.getenv(
            agent_params_config["model_env_var"], agent_params_config["default_model"]
        ),
...
@click.option(
    "--agent-name",
    required=True,
    help="The name of the agent to run (e.g., a2a_planner, a2a_custom_tools). Corresponds to the directory name.",
)
...
def main(agent_name, host, port):
    ...
    if port is None:
        try:
            config_module_path = f"agents.a2a_llama_stack.agents.{
                agent_name.replace(
                    '-', '_')}.config"
            config_module = importlib.import_module(config_module_path)
    ...
    logging.info(
        f"Attempting to start server for agent: {agent_name} on {host}:{effective_port}"
    )
