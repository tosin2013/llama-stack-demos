...
@client_tool
def monitor_repository_changes_tool(repository_urls: str, time_period: str = "7d") -> str:
    """
    ...
    try:
        ...
        for repo_data in changes_report["repositories"]:
            ...
            if repo_data["changes"]:
                ...
                for change in repo_data["changes"]:
                    impact_info = CHANGE_IMPACT_LEVELS.get(change["type"], {"priority": "low"})
                    priority_emoji = {"high": "🔴", "medium": "🟡", "low": "🟢"}.get(impact_info["priority"], "⚪")
        ...
        for repo_data in changes_report["repositories"]:
            for change in repo_data["changes"]:
                ...
                if impact_info["priority"] == "high":
                    high_priority_changes.append(f"{repo_data['repository']}: {change['description']}")
                elif impact_info["priority"] == "medium":
                    medium_priority_changes.append(f"{repo_data['repository']}: {change['description']}")
        ...
        return f"Error monitoring repository changes: {str(e)}. Please check the repository URLs and try again."


@client_tool
def analyze_impact_tool(changes_description: str, workshop_sections: str = "all") -> str:
    """
    ...
    try:
        ...
        if workshop_sections.lower() == "all":
            sections_to_analyze = ["introduction", "setup", "exercises", "troubleshooting", "resources", "deployment"]
        else:
            sections_to_analyze = [section.strip() for section in workshop_sections.split(',')]
        ...
        if not detected_changes:
            ...
            if any(word in changes_lower for word in ["security", "vulnerability", "cve"]):
        ...
        return f"Impact analysis completed for {len(sections_to_analyze)} sections. Use create_update_proposal_tool for specific proposals."
        
    except Exception as e:
        logger.error(f"Error in analyze_impact_tool: {e}")
        return f"Error analyzing impact: {str(e)}. Please check your inputs and try again."


@client_tool
def create_update_proposal_tool(impact_analysis: str, section_name: str, update_type: str = "content") -> str:
    """
    ...
    try:
        ...
        proposal_id = f"WS-{datetime.now().strftime('%Y%m%d')}-{section_name.upper()[:3]}-{update_type.upper()[:3]}"
        ...
        return f"Error creating update proposal: {str(e)}. Please check your inputs and try again."


@client_tool
def monitor_external_sources_tool(sources_config: str, monitoring_frequency: str = "daily") -> str:
    """
    ...
    try:
        ...
        try:
            ...
            sources = {
                "documentation_sites": [sources_config] if sources_config.startswith("http") else [],
                "pdf_documents": [],
                "api_endpoints": []
            }
        ...
        report_parts = [
            ...
            f"**Report Generated**: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
            f"**Sources Monitored**: {len(sources.get('documentation_sites', [])) + len(sources.get('pdf_documents', [])) + len(sources.get('api_endpoints', []))}",
            "",
        ...
        report_parts.extend([
            ...
            f"*Frequency: {monitoring_frequency} | Sources: {len(sources.get('documentation_sites', [])) + len(sources.get('pdf_documents', [])) + len(sources.get('api_endpoints', []))}*"
        ])
        ...
        return f"Error monitoring external sources: {str(e)}. Please check your configuration and try again."
