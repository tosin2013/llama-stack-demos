...
def fetch_repository_structure(repository_url: str) -> dict:
    ...
    try:
        ...
        if (
            github_token
            and github_token != "placeholder-github-token"
        ...
        if response.status_code == 200:
            ...
            for readme_name in ["README.md", "README.rst", "README.txt", "README"]:
                if readme_name in files:
                    ...
                    readme_response = requests.get(
                        readme_url, headers=headers, timeout=30
                    )
            ...
            return {
                "error": f"GitHub API request failed: {
                    response.status_code}",
                "success": False,
            }
...
WORKSHOP_INDICATORS = {
    ...
    # Red Hat SE RTO template
    "legacy_rto": ["default-site.yml", "demo-site.yml", "content/"],
...
def detect_existing_workshop(repository_url: str, repo_structure: dict = None) -> dict:
    ...
    # Check for workshop indicators in repository name (but don't rely solely
    # on this)
    name_suggests_workshop = any(
        indicator in repo_lower
        for indicator in ["workshop", "lab", "tutorial", "guide"]
    )
    if name_suggests_workshop:
        indicators_found.append(
            "Workshop keyword in repository name (requires framework validation)"
        )

    # If we have actual repository structure, analyze it for ACTUAL workshop
    # frameworks
    if repo_structure:
        files = repo_structure.get("files", [])
        directories = repo_structure.get("directories", [])

        # Check for ACTUAL workshop framework indicators (not just content
        # organization)
        for framework, indicators in WORKSHOP_INDICATORS.items():
            ...
            for indicator in indicators:
                if indicator.endswith("/"):
                    # Directory indicator
                    if any(
                        indicator.rstrip("/") in dir_name for dir_name in directories
                    ):
                        framework_score += 1
            ...
            if framework_score >= 1:  # Lowered threshold but still require actual files
                # Check if we found actual framework files (not just
                # directories)
        ...
        # If no framework detected but has workshop-like organization, it's
        ...
        if not workshop_detected and name_suggests_workshop:
            ...
            tutorial_patterns = any(
                dir_name.startswith(
                    ("01-", "02-", "03-", "module", "chapter", "lab", "exercise")
                )
            ...
            if tutorial_patterns:
                ...
                indicators_found.append(
                    "No formal workshop framework found - needs Showroom template conversion"
                )
    ...
    if not workshop_detected:
        if "openshift-bare-metal-deployment-workshop" in repo:
            # This is a known structured workshop - check if it has proper
            # framework
            if any(
                file in files
                for file in ["antora.yml", "showroom.yml", "default-site.yml"]
            ):
                workshop_detected = True
            ...
            keyword in repo_lower for keyword in ["healthcare-ml", "genetic-predictor"]
        ):
            ...
            # DDD Hexagonal Workshop - tutorial content, not framework-based
            # workshop
            workshop_detected = False
            workshop_type = "tutorial_content"
            indicators_found.extend(
...
@client_tool
def analyze_repository_tool(
    repository_url: str, analysis_depth: str = "standard"
) -> str:
    """
    ...
    try:
        ...
        if not repo_structure.get("success", False):
            logger.warning(
                f"Failed to fetch repository structure: {
                    repo_structure.get(
                        'error', 'Unknown error')}"
            )
        ...
        workshop_detection.get("workflow_recommendation", "creation")
        report_parts.extend(
            [
                "",
        ...
        template = WORKSHOP_TEMPLATES.get(
            primary_category, WORKSHOP_TEMPLATES["web_application"]
        )
        ...
        return f"Error analyzing repository '{repository_url}': {
            str(e)}. Please check the URL and try again."


@client_tool
def generate_workshop_structure_tool(
    repository_analysis: str,
    workshop_type: str = "auto",
    ...
    try:
        ...
        # Get template
        template = WORKSHOP_TEMPLATES.get(
            workshop_type, WORKSHOP_TEMPLATES["web_application"]
        )
        ...
        workshop_objectives = objectives.get(
            workshop_type, objectives["web_application"]
        )
        for obj in workshop_objectives:
            structure_parts.append(f"- {obj}")

        structure_parts.extend(["", "## 📋 Workshop Sections", ""])
        ...
        for i, section in enumerate(template["sections"], 1):
            section_info = section_details.get(
                section,
                {
                    "duration": "30-45 minutes",
        ...
        return f"Error generating workshop structure: {
            str(e)}. Please check your inputs and try again."


@client_tool
def validate_workshop_requirements_tool(
    repository_url: str, workshop_structure: str = ""
) -> str:
    """
    ...
    try:
        ...
        return f"Error validating workshop requirements for '{repository_url}': {
            str(e)}. Please check the URL and try again."
